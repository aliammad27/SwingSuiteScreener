# SwingSuiteScreener: Claude Code Product and Engineering Specification

## 0. Repository and Delivery Contract

Repository name: `SwingSuiteScreener`

Claude Code is responsible for building the complete version one application in this repository.

Git workflow:

1. Work on the active development branch or `main` unless instructed otherwise. Inspect `git branch` to confirm the current branch before editing.
...
9. Push the completed and tested implementation to `origin/main` or the current development branch.
...
The connected Robinhood MCP server (`mcp__robinhood__*`) may be used only for read-only exploration and validation while developing. The production application must use a direct Alpaca provider implementation and must not depend on any MCP server or interactive agent at runtime.
...
The Telegram token is a secret. Never place a real token in `CLAUDE.md`, source code, YAML, reports, logs, test fixtures, screenshots, prompts, or committed files. Load it only from the local `.env` file.
...
Never print the bot token in logs, reports, terminal output, exception messages, test output, or Claude Code session output.
